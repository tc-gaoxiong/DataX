...
public class HdfsHelper {
  ...
  public static MutablePair<Text, Boolean> transportOneRecord(
    ...
    MutablePair<List<Object>, Boolean> transportResultList = transportOneRecord(
            record,
      ...
      switch (colType.toLowerCase()) {
        ...
        default:
          if (ColumnTypeUtil.isDecimalType(colType)) {
            ColumnTypeUtil.DecimalInfo decimalInfo = ColumnTypeUtil.getDecimalInfo(
                    colType,
                    PARQUET_DEFAULT_DECIMAL_INFO);
  ...
  public void getFileSystem(String defaultFS, Configuration taskConfig) {
    ...
    try {
      ...
      String message = String.format(
              "获取FileSystem时发生网络IO异常,请检查您的网络是否正常!HDFS地址：[%s]",
  ...
  public String[] hdfsDirList(String dir) {
    ...
    try {
      ...
      String message = String.format(
              "获取目录[%s]文件列表时发生网络IO异常,请检查您的网络是否正常！",
              dir);
      LOG.error(message);
      throw DataXException.asDataXException(HdfsWriterErrorCode.CONNECT_HDFS_IO_ERROR, e);
    }
  ...
  public boolean isPathexists(String filePath) {
    ...
    try {
      ...
      String message = String.format(
              "判断文件路径[%s]是否存在时发生网络IO异常,请检查您的网络是否正常！",
  ...
  public boolean isPathDir(String filePath) {
    ...
    try {
      ...
      String message = String.format(
              "判断路径[%s]是否是目录时发生网络IO异常,请检查您的网络是否正常！",
              filePath);
      LOG.error(message);
      throw DataXException.asDataXException(HdfsWriterErrorCode.CONNECT_HDFS_IO_ERROR, e);
    }
  ...
  public void renameFile(HashSet<String> tmpFiles, HashSet<String> endFiles) {
    ...
    if (tmpFiles.size() != endFiles.size()) {
      ...
      try {
        for (Iterator it1 = tmpFiles.iterator(), it2 = endFiles.iterator();
          ...
          if (fileLen > 0) {
            ...
            if (!renameTag) {
              ...
              throw DataXException.asDataXException(
                      HdfsWriterErrorCode.HDFS_RENAME_FILE_ERROR,
                      message);
            }
  ...
  public FSDataOutputStream getOutputStream(String path) {
    ...
    try {
      ...
      String message = String.format(
              "Create an FSDataOutputStream at the indicated Path[%s] failed: [%s]",
              "message:path =" + path);
      LOG.error(message);
  ...
  public void textFileStartWrite(
    ...
    try {
      RecordWriter writer = outFormat.getRecordWriter(
              fileSystem,
              conf,
              outputPath.toString(),
              Reporter.NULL);
      ...
      while ((record = lineReceiver.getFromReader()) != null) {
        MutablePair<Text, Boolean> transportResult = transportOneRecord(
                record,
                fieldDelimiter,
                columns,
                taskPluginCollector);
  ...
  public void orcFileStartWrite(
    ...
    try {
      ...
      Record record = null;
      while ((record = lineReceiver.getFromReader()) != null) {
        MutablePair<List<Object>, Boolean> transportResult = transportOneRecord(
                record,
                columns,
                taskPluginCollector);
        if (!transportResult.getRight()) {
          writer.write(
                  NullWritable.get(),
                  orcSerde.serialize(transportResult.getLeft(), inspector));
        }
      }
      writer.close(Reporter.NULL);
    } catch (Exception e) {
  ...
  public List<ObjectInspector> getColumnTypeInspectors(List<Configuration> columns) {
    ...
    for (Configuration eachColumnConf : columns) {
      SupportHiveDataType columnType = SupportHiveDataType.valueOf(eachColumnConf
              .getString(Key.TYPE)
              .toUpperCase());
      ObjectInspector objectInspector = null;
      switch (columnType) {
        case TINYINT:
          objectInspector = ObjectInspectorFactory.getReflectionObjectInspector(
                  Byte.class,
  ...
  public void parquetFileStartWrite(
    ...
    try {
      ...
      String message = String.format(
              "Error parsing the Schema string [%s] into MessageType",
              schema);
      LOG.error(message);
      throw DataXException.asDataXException(
              HdfsWriterErrorCode.PARSE_MESSAGE_TYPE_FROM_SCHEMA_ERROR,
              e);
    }
    ...
    LOG.info(
            "The compression codec used for parquet writing is: {}",
            compressionCodecName,
            compress);
    try {
      proccessor = new ParquetFileProccessor(
              outputPath,
              messageType,
              compressionCodecName,
              false,
      ...
      String message = String.format(
              "Initializing ParquetFileProccessor based on Schema[%s] failed.",
              schema);
      LOG.error(message);
    ...
    try {
      ...
      String message = String.format(
              "An exception occurred while writing the file file [%s]",
              fileName);
      LOG.error(message);
      Path path = new Path(fileName);
      deleteDir(path.getParent());
