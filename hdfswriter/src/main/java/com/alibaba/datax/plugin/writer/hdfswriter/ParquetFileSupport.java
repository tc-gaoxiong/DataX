...
public class ParquetFileSupport extends WriteSupport<Record> {
  ...
  public ParquetFileSupport(
          MessageType schema,
          com.alibaba.datax.common.util.Configuration taskConfig,
            ...
            if (this.useRawDataTransf) {
              ...
              switch (columnDescriptor.getType()) {
                ...
                case INT32:
                  ...
                  if (originalType != null && StringUtils.equalsIgnoreCase(
                          "DATE",
                          originalType.name())) {
                    int realVal = (int) (new java.sql.Date(Long.parseLong(rawData))
                            .toLocalDate()
                            .toEpochDay());
                ...
                case FIXED_LEN_BYTE_ARRAY:
                  ...
                  if (primitiveType.getDecimalMetadata() != null) {
                    // decimal
                    recordConsumer.addBinary(decimalToBinary(
                            value,
                ...
                switch (columnDescriptor.getType()) {
                  ...
                  case INT32:
                    ...
                    if (originalType != null && StringUtils.equalsIgnoreCase(
                            "DATE",
                            originalType.name())) {
                      int realVal = (int) (new java.sql.Date(value.asLong())
                              .toLocalDate()
                              .toEpochDay());
                      recordConsumer.addInteger(realVal);
                    } else {
    ...
    switch (primitiveType.getPrimitiveTypeName()) {
      ...
      case INT32:
        if (value instanceof Integer) {
          ...
          LimitLogger.limit(
                  "dirtyDataHiveWriterParquet",
                  TimeUnit.MINUTES.toMillis(1),
                  () -> LOGGER.warn(
                          "dirtyDataHiveWriterParquet {}",
                          String.format(
                                  "Invalid value: %s(clazz: %s) for field: %s",
                                  value,
                                  value.getClass(),
                                  type.getName())));
        }
