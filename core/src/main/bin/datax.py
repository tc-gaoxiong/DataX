...
DEFAULT_JVM = (
    "-Xms1g -Xmx1g -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=%s/log"
    % (DATAX_HOME)
)
...
def suicide(signum, e):
    ...
    if ispy2:
        (
            print >> sys.stderr,
            "[Error] DataX receive unexpected signal %d, starts to suicide." % (signum),
        )
    else:
        print(
            "[Error] DataX receive unexpected signal %d, starts to suicide." % (signum),
...
def generateJobConfigTemplate(reader, writer):
    readerRef = (
        "Please refer to the %s document:\n     https://github.com/alibaba/DataX/blob/master/%s/doc/%s.md \n"
    ...
    try:
        ...
        print(
            "Read reader[%s] template error: can't find file %s"
    ...
    try:
        ...
        print(
            "Read writer[%s] template error: : can't find file %s"
...
def buildStartCommand(options, args):
    ...
    jobParams = ("-Dlog.file.name=%s") % (
        os.path.splitext(os.path.basename(jobResource))[0]
    )
    if options.params:
