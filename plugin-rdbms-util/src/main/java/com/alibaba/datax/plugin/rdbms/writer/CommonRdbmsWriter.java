...
public class CommonRdbmsWriter {
  ...
  public static class Job {
    ...
    public void init(Configuration originalConfig) {
      ...
      LOG.debug(
              "After job init(), originalConfig now is:[\n{}\n]",
    ...
    public void privilegeValid(Configuration originalConfig, DataBaseType dataBaseType) {
      ...
      for (int i = 0, len = connections.size(); i < len; i++) {
        ...
        if (!hasInsertPri) {
          throw RdbmsException.asInsertPriException(
                  dataBaseType,
                  originalConfig.getString(Key.USERNAME),
                  jdbcUrl);
        }
        ...
        if (DBUtil.needCheckDeletePrivilege(originalConfig)) {
          boolean hasDeletePri = DBUtil.checkDeletePrivilege(
                  ...
                  expandedTables);
          if (!hasDeletePri) {
            throw RdbmsException.asDeletePriException(
                    dataBaseType,
                    originalConfig.getString(Key.USERNAME),
                    jdbcUrl);
          }
    ...
    public void prepare(Configuration originalConfig) {
      ...
      LOG.debug(
              "After job prepare(), originalConfig now is:[\n{}\n]",
              originalConfig.toJSON());
    }
  ...
  public static class Task {
    ...
    public void init(Configuration writerSliceConfig) {
      ...
      this.batchByteSize = writerSliceConfig.getInt(
              Key.BATCH_BYTE_SIZE,
        ...
        for (Record record : buffer) {
          try {
            ...
            if (needToDumpRecord()) {
              ...
              LOG.warn(
                      "Insert fatal error SqlState ={}, errorCode = {}, {}",
                      e.getSQLState(),
                      e.getErrorCode(),
                      e);
            }
