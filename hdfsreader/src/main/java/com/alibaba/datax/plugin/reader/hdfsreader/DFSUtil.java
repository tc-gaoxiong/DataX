...
public class DFSUtil {
  ...
  private void kerberosAuthentication(String kerberosPrincipal, String kerberosKeytabFilePath) {
    if (haveKerberos && StringUtils.isNotBlank(this.kerberosPrincipal) && StringUtils.isNotBlank(
  ...
  public HashSet<String> getAllFiles(
          ...
          Boolean skipEmptyOrcFile,
          Integer orcFileEmptySize) {
  ...
  public HashSet<String> getHDFSAllFiles(String hdfsPath) {
    ...
    try {
        ...
        for (FileStatus f : stats) {
          if (f.isFile()) {
            ...
            if (fileLength == 0) {
              ...
              LOG.warn(message);
            } else if (BooleanUtils.isTrue(this.skipEmptyOrcFile) && this.orcFileEmptySize != null
                    && fileLength <= this.orcFileEmptySize) {
              String message = String.format(
                      "The orc file [%s] is empty, file size: %s, DataX will skip it !",
                      f.getPath().toString(),
                      fileLength);
              LOG.warn(message);
            } else {
      ...
      String message = String.format(
              "无法读取路径[%s]下的所有文件,请确认您的配置项fs.defaultFS, path的值是否正确，" +
  ...
  private HashSet<String> getHDFSAllFilesNORegex(String path, FileSystem hdfs) throws IOException {
    ...
    for (FileStatus f : stats) {
      ...
      if (f.isDirectory()) {
        ...
        if (fileLength == 0) {
          String message = String.format(
                  "The file [%s] is empty, DataX will skip it !",
                  f.getPath().toString());
          LOG.warn(message);
          continue;
        } else if (BooleanUtils.isTrue(this.skipEmptyOrcFile) && this.orcFileEmptySize != null
  ...
  private void addSourceFileByType(String filePath) {
    ...
    if (isMatchedFileType) {
      LOG.info(String.format(
              "[%s]是[%s]类型的文件, 将该文件加入source files列表",
              filePath,
              this.specifiedFileType));
      sourceHDFSAllFilesList.add(filePath);
    } else {
      String message = String.format(
              "文件[%s]的类型与用户配置的fileType类型不一致，" +
  ...
  public void sequenceFileStartRead(
          String sourceSequenceFilePath, Configuration readerSliceConfig,
    ...
    try {
      ...
      while (reader.next(key, value)) {
        if (StringUtils.isNotBlank(value.toString())) {
          UnstructuredStorageReaderUtil.transportOneRecord(
                  recordSender,
      ...
      String message = String.format(
              "SequenceFile.Reader读取文件[%s]时出错",
              sourceSequenceFilePath);
      LOG.error(message);
      throw DataXException.asDataXException(
              HdfsReaderErrorCode.READ_SEQUENCEFILE_ERROR,
              message,
              e);
    } finally {
  ...
  public void rcFileStartRead(
          String sourceRcFilePath, Configuration readerSliceConfig,
          RecordSender recordSender, TaskPluginCollector taskPluginCollector) {
  ...
  public void orcFileStartRead(
          String sourceOrcFilePath, Configuration readerSliceConfig,
          RecordSender recordSender, TaskPluginCollector taskPluginCollector) {
    ...
    if (columnIndexMax >= 0) {
      ...
      try {
        ...
        try {
          ...
          if (Boolean.TRUE.equals(this.skipEmptyOrcFile)) {
            ...
            if (isOrcFileEmptyException) {
              LOG.info(
                      "skipEmptyOrcFile: true, \"{}\" is an empty orc file, skip it!",
                      sourceOrcFilePath);
              return;
            }
          }
        ...
        for (InputSplit split : splits) {
          {
            ...
            while (reader.next(key, value)) {
              ...
              List<ColumnEntry> hivePartitionColumnEntrys = UnstructuredStorageReaderUtil.getListColumnEntry(
                      readerSliceConfig,
                      com.alibaba.datax.plugin.unstructuredstorage.reader.Key.HIVE_PARTION_COLUMN);
              ArrayList<Column> hivePartitionColumns = new ArrayList<>();
              hivePartitionColumns = UnstructuredStorageReaderUtil.getHivePartitionColumns(
                      sourceOrcFilePath,
                      hivePartitionColumnEntrys);
              transportOneRecord(
                      column, recordFields, recordSender,
        ...
        String message = String.format(
                "从orcfile文件路径[%s]中读取数据发生异常，请联系系统管理员。"
                , sourceOrcFilePath);
        LOG.error(message);
  ...
  private boolean checkIsOrcEmptyFileExecption(Exception e) {
    ...
    if (fullStackTrace.contains("org.apache.orc.impl.ReaderImpl.getRawDataSizeOfColumn")
            && fullStackTrace.contains(
  ...
  private Record transportOneRecord(
          List<ColumnEntry> columnConfigs,
          List<Object> recordFields
          ,
          RecordSender recordSender,
          TaskPluginCollector taskPluginCollector,
          boolean isReadAllColumns,
          String nullFormat,
          ArrayList<Column> hiveParitionColumns) {
  ...
  private int getMaxIndex(List<ColumnEntry> columnConfigs) {
    ...
    for (ColumnEntry columnConfig : columnConfigs) {
      ...
      if (columnIndex != null && columnIndex < 0) {
        String message = String.format(
                "您column中配置的index不能小于0，请修改为正确的index,column配置:%s",
  ...
  public boolean checkHdfsFileType(String filepath, String specifiedFileType) {
    ...
    try {
      ...
      String message = String.format(
              "检查文件[%s]类型失败，目前支持ORC,SEQUENCE,RCFile,TEXT,CSV五种格式的文件," +
  ...
  public void parquetFileStartRead(
    ...
    if (StringUtils.isNotBlank(schemaString)) {
      ...
      LOG.info(
              "Parquet schema parsed from: {} , schema is {}",
    ...
    try {
      ...
      String message = String.format(
              "Error parsing to MessageType via Schema string [%s]",
              schemaString);
      LOG.error(message);
      throw DataXException.asDataXException(
              HdfsReaderErrorCode.PARSE_MESSAGE_TYPE_FROM_SCHEMA_ERROR,
              e);
    }
    List<ColumnEntry> column = UnstructuredStorageReaderUtil.getListColumnEntry(
            readerSliceConfig,
            com.alibaba.datax.plugin.unstructuredstorage.reader.Key.COLUMN);
    String nullFormat = readerSliceConfig.getString(com.alibaba.datax.plugin.unstructuredstorage.reader.Key.NULL_FORMAT);
     ...
     */
    boolean supportAddMiddleColumn = readerSliceConfig.getBool(
            Key.SUPPORT_ADD_MIDDLE_COLUMN,
            false);

    boolean printNullValueException = readerSliceConfig.getBool("printNullValueException", false);
    List<Integer> ignoreIndex = readerSliceConfig.getList(
            "ignoreIndex",
    ...
    try {
      ...
      ParquetReader.Builder parquetReaderBuilder = ParquetReader.builder(
              readSupport,
              parquetFilePath);
      ...
      if (supportAddMiddleColumn) {
        boolean nonName = column
                .stream()
                .anyMatch(columnEntry -> StringUtils.isEmpty(columnEntry.getName()));
        if (nonName) {
          throw new DataXException("You configured column item without name, please correct it");
        }
        List<org.apache.parquet.schema.Type> parquetFileFields = getParquetFileFields(
                parquetFilePath,
                hadoopConf);
        schemaFieldList = parquetFileFields
                .stream()
        ...
        column.forEach(columnEntry -> finalColNameIndexMap.put(
        ...
        LOG.info(
                "SupportAddMiddleColumn is true, fields from parquet file is {}, " +
                        "colNameIndexMap is {}",
                JSON.toJSONString(schemaFieldList),
                JSON.toJSONString(colNameIndexMap));
              ...
              if (null != ignoreIndex && !ignoreIndex.isEmpty() && ignoreIndex.contains(j)) {
                ...
                if (supportAddMiddleColumn) {
                  if (!colNameIndexMap.containsKey(j)) {
                    ...
                    data = DFSUtil.this.readFields(
                            g,
                            parquetTypes.get(indexMap.get(j)),
                            indexMap.get(j),
                            parquetMetaMap,
                            isUtcTimestamp);
                  }
  ...
  private List<org.apache.parquet.schema.Type> getParquetFileFields(
          Path filePath,
    ...
    try (org.apache.parquet.hadoop.ParquetFileReader reader = org.apache.parquet.hadoop.ParquetFileReader.open(
      ...
      org.apache.parquet.schema.MessageType schema = reader
              .getFooter()
              .getFileMetaData()
              .getSchema();
      ...
      throw new DataXException(String.format(
              "Fetch parquet field error, msg is %s",
              e.getMessage()));
    }
  }

  private String getParquetSchema(
          String sourceParquetFilePath,
          org.apache.hadoop.conf.Configuration hadoopConf) {
    GroupReadSupport readSupport = new GroupReadSupport();
    ParquetReader.Builder parquetReaderBuilder = ParquetReader.builder(
            readSupport,
            new Path(sourceParquetFilePath));
    ParquetReader<Group> reader = null;
    try {
      parquetReaderBuilder.withConf(hadoopConf);
      reader = parquetReaderBuilder.build();
  ...
  private org.apache.parquet.schema.OriginalType getOriginalType(
          org.apache.parquet.schema.Type type,
          Map<String, ParquetMeta> parquetMetaMap) {
    ParquetMeta meta = parquetMetaMap.get(type.getName());
    return meta.getOriginalType();
  }

  private org.apache.parquet.schema.PrimitiveType asPrimitiveType(
          org.apache.parquet.schema.Type type,
          Map<String, ParquetMeta> parquetMetaMap) {
    ParquetMeta meta = parquetMetaMap.get(type.getName());
  ...
  private Object readFields(
    ...
    if (this.getOriginalType(type, parquetMetaMap) == org.apache.parquet.schema.OriginalType.MAP) {
        ...
        for (int k = 0; k < j; k++) {
          ...
          if (2 != parquetTypesK.size()) {
            ...
            throw new RuntimeException(String.format(
                    "bad parquet map type: %s",
                    groupData.getValueToString(index, 0)));
          }
          Object subDataKey = this.readFields(
                  groupDataK,
                  parquetTypesK.get(0),
                  0,
                  parquetMetaMap,
                  isUtcTimestamp);
          Object subDataValue = this.readFields(
                  groupDataK,
                  parquetTypesK.get(1),
                  1,
                  parquetMetaMap,
      ...
      return data;
    } else if (this.getOriginalType(type, parquetMetaMap)
            == org.apache.parquet.schema.OriginalType.MAP_KEY_VALUE) {
      Group groupData = g.getGroup(index, 0);
      List<org.apache.parquet.schema.Type> parquetTypes = groupData.getType().getFields();
      JSONObject data = new JSONObject();
      for (int i = 0; i < parquetTypes.size(); i++) {
      ...
      return data;
            ...
            == org.apache.parquet.schema.OriginalType.LIST) {
      Group groupData = g.getGroup(index, 0);
      List<org.apache.parquet.schema.Type> parquetTypes = groupData.getType().getFields();
      JSONArray data = new JSONArray();
      for (int i = 0; i < parquetTypes.size(); i++) {
        Object subData = this.readFields(
                groupData,
                parquetTypes.get(i),
                i,
                parquetMetaMap,
      ...
      return data;
            ...
            == org.apache.parquet.schema.OriginalType.DECIMAL) {
      Binary binaryDate = g.getBinary(index, 0);
      if (null == binaryDate) {
        return null;
      } else {
        org.apache.hadoop.hive.serde2.io.HiveDecimalWritable decimalWritable = new org.apache.hadoop.hive.serde2.io.HiveDecimalWritable(
                binaryDate.getBytes(),
                this.asPrimitiveType(type, parquetMetaMap).getDecimalMetadata().getScale());
      ...
      if (type.isPrimitive()) {
        PrimitiveType.PrimitiveTypeName primitiveTypeName = this
                .asPrimitiveType(type, parquetMetaMap)
