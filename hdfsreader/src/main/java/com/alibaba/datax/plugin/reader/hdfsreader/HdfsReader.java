...
public class HdfsReader extends Reader {
  ...
  public static class Job extends Reader.Job {
    ...
    public void validate() {
      this.readerOriginConfig.getNecessaryValue(
              Key.DEFAULT_FS,
              HdfsReaderErrorCode.DEFAULT_FS_NOT_FIND_ERROR);

      // path check
      String pathInString = this.readerOriginConfig.getNecessaryValue(
              Key.PATH,
      ...
      specifiedFileType = this.readerOriginConfig.getNecessaryValue(
              Key.FILETYPE,
              HdfsReaderErrorCode.REQUIRED_VALUE);
      if (!specifiedFileType.equalsIgnoreCase(Constant.ORC) &&
        ...
        String message =
                "HdfsReader插件目前支持ORC, TEXT, CSV, SEQUENCE, RC, PARQUET 六种格式的文件," +
      ...
      encoding = this.readerOriginConfig.getString(
    ...
    public void prepare() {
      ...
      this.sourceFiles = dfsUtil.getAllFiles(
              path,
              specifiedFileType,
              skipEmptyOrcFile,
              orcFileEmptySize);
    ...
    public List<Configuration> split(int adviceNumber) {
      ...
      if (0 == splitNumber) {
        throw DataXException.asDataXException(
                HdfsReaderErrorCode.EMPTY_DIR_EXCEPTION,
                String.format(
                        "未能找到待读取的文件,请确认您的配置项path: %s",
                        this.readerOriginConfig.getString(Key.PATH)));
      }

      List<List<String>> splitedSourceFiles = this.splitSourceFiles(
              new ArrayList<String>(this.sourceFiles),
              splitNumber);
  ...
  public static class Task extends Reader.Task {
    ...
    public void init() {
      ...
      this.specifiedFileType = this.taskConfig.getNecessaryValue(
              Key.FILETYPE,
              HdfsReaderErrorCode.REQUIRED_VALUE);
      this.encoding = this.taskConfig.getString(
              com.alibaba.datax.plugin.unstructuredstorage.reader.Key.ENCODING,
              "UTF-8");
      this.dfsUtil = new DFSUtil(this.taskConfig);
      this.bufferSize = this.taskConfig.getInt(
              com.alibaba.datax.plugin.unstructuredstorage.reader.Key.BUFFER_SIZE,
